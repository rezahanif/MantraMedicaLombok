...
//
// Usage:
//   import { useBookingToast, BookingToastContainer } from "@/components/shared/Bookingtoast";
//
//   // 1. Mount the container once (e.g. in layout.tsx or page.tsx):
//   <BookingToastContainer />
//
//   // 2. In any component, trigger toasts:
//   const toast = useBookingToast();
//   toast.success("Booking confirmed! We'll contact you via WhatsApp.");
//   toast.error("Please fill in all required fields.");
//   toast.warning("Something looks off — please check your details.");
//
// ── Auto-dismisses after 3 s. User can also dismiss manually.
// ── Desktop: slides in from top-right. Mobile (≤499px): slides up from bottom.
...
function ToastItem({
  ...
  /* Dismiss helper — triggers exit animation then removes */
  const dismiss = useCallback(() => {
    setExiting(true);
    setTimeout(() => onRemove(toast.id), 340);
  }, [toast.id, onRemove]);

  /* Auto-dismiss when progress hits 0 */
  useEffect(() => {
  ...
  return (
    <>
      <style>{`
        ...
        /* Desktop */
        .bk-toast {
          animation: toast-in-desktop 0.36s cubic-bezier(0.32, 0.72, 0, 1) forwards;
        }
        .bk-toast.bk-exit {
          animation: toast-out-desktop 0.32s ease forwards;
        }

        /* Mobile */
        @media (max-width: 499px) {
          .bk-toast {
            animation: toast-in-mobile 0.36s cubic-bezier(0.32, 0.72, 0, 1) forwards;
          }
          .bk-toast.bk-exit {
            animation: toast-out-mobile 0.32s ease forwards;
          }
        }
      ...
      >
        ...
        {/* Progress bar — depletes left→right over 3 s */}
        <div
          aria-hidden="true"
          style={{
            position:   "absolute",
...
/* ─── Container + Provider ───────────────────────────────── */
export function BookingToastContainer() {
